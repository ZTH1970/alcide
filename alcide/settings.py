...
# Django settings for alcide project.

import os
from django.core.exceptions import ImproperlyConfigured
from logging.handlers import SysLogHandler

PROJECT_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'alcide')
...
DATABASES = {
    'default': {
        ...
        'NAME': os.path.join(PROJECT_PATH, 'alcide.sqlite3'),                      # Or path to database file if using sqlite3.
...
USE_L10N = True
FORMAT_MODULE_PATH = 'alcide.formats'
...
MIDDLEWARE_CLASSES = (
    ...
    'alcide.middleware.request.GlobalRequestMiddleware',
...
ROOT_URLCONF = 'alcide.urls'

# Python dotted path to the WSGI application used by Django's runserver.
WSGI_APPLICATION = 'alcide.wsgi.application'
...
LOGGING = {
    ...
    'formatters': {
        ...
        'syslog': {
            'format': 'alcide (pid=%(process)d) %(levelname)s %(name)s: %(message)s',
        },
    },
    'handlers': {
        'mail_admins': {
...
AJAX_LOOKUP_CHANNELS = {
    ...
    'worker' : ('alcide.personnes.lookup', 'WorkerLookup'),
    'intervenant' : ('alcide.personnes.lookup', 'IntervenantLookup'),
    ...
    'patientrecord' : ('alcide.dossiers.lookups', 'PatientRecordLookup'),
    #'school' : {'model':'ressources.School', 'search_field':'name'},
    'school' : ('alcide.ressources.lookups', 'SchoolLookup'),
    'addresses' : ('alcide.dossiers.lookups', 'PatientAddressLookup'),
    'worker-or-group' : ('alcide.ressources.lookups', 'WorkerOrGroupLookup'),
    'all-worker-or-group' : ('alcide.ressources.lookups', 'AllWorkerOrGroupLookup'),
}
...
# B2_TRANSMISSION = {
#    'output_directory': '/var/lib/alcide/B2/',
#    # B2 informations
